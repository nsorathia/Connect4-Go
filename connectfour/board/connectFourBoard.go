...
type ConnectFourBoard struct {
	Grid                [][]enums.Token
	LastMoveColumnIndex int //Column Index of Last Player's Move
}

//NewBoard creates a ConnectFourBoard with default dimensions:  6 rows and 7 columns
func NewBoard() (ConnectFourBoard, error) {
	return CreateNewBoard(6, 7)
}
...
func CreateNewBoard(rows, columns int) (ConnectFourBoard, error) {
	...
	return ConnectFourBoard{Grid: grid, LastMoveColumnIndex: -1}, nil
}

//Rows is a method that returns the number of rows in the ConnectFourBoard
func (b *ConnectFourBoard) Rows() int {
	return len(b.Grid)
}

//Columns is a method that returns the number of Columns for the board.
func (b *ConnectFourBoard) Columns() int {
	return len(b.Grid[0])
}

//Clone is a method whihc returns a deep copy of the C4 board
func (b *ConnectFourBoard) Clone() ConnectFourBoard {
	clone := ConnectFourBoard{
		Grid:                b.Grid,
		LastMoveColumnIndex: b.LastMoveColumnIndex,
	}
	return clone
}

//DropPlayerToken is a method that updates a C4 Board with a player's token given teh Column Number
func (b *ConnectFourBoard) DropPlayerToken(columnNumber int, token enums.Token) error {

	if columnNumber < 1 || columnNumber > b.Columns() {
	...
	for i := b.Rows() - 1; i >= 0; i-- {
		if b.Grid[i][moveIndex] == enums.Empty {
			b.Grid[i][moveIndex] = token
			b.LastMoveColumnIndex = moveIndex
			break
		}
	}
...
//IsLastMoveWin is a method which determines if the last players Move was a win
func (b *ConnectFourBoard) IsLastMoveWin() bool {
...
func (b *ConnectFourBoard) ToString() string {
	...
	for rows := range b.Grid {
		for columns := range b.Grid[rows] {
			var marker string

			switch token := b.Grid[rows][columns]; token {
			case enums.Red:
				marker = " X "
			case enums.Yellow:
				marker = " O "
...
func (b *ConnectFourBoard) isColumnFull(column int) bool {
	isFull := false
	if b.Grid[0][column] != enums.Empty {
		isFull = true
	}
...
func (b *ConnectFourBoard) lastMoveRowIndex() int {
	...
	i := 0;
	for ; i < numOfRows; i++ {
		if b.Grid[i][b.LastMoveColumnIndex] != enums.Empty {
			break
		}
	}
...
func (b *ConnectFourBoard) isHorizontalWin(rowIndex, columnIndex int) bool {
	...
	rightBound := min(b.Columns()-1, columnIndex+3)
	token := b.Grid[rowIndex][columnIndex]
	count := 0

	for i := leftBound; i <= rightBound; i++ {
		if b.Grid[rowIndex][i] == token {
			count++
...
func (b *ConnectFourBoard) isVerticalWin(rowIndex, columnIndex int) bool {
	lowBound := rowIndex
	highBound := min(rowIndex + 3, b.Rows() - 1)
	token := b.Grid[rowIndex][columnIndex]
	count := 0
